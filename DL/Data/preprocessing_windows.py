...
for idx, i in tqdm(enumerate(label_file_path)):
    ...
    file = json.load(open(i , encoding="utf-8"))
    ...
    if file["Bounding"][0]["Drawing"] != "POLYGON":
        ...
        # 윈도우에서 opencv는 한글경로를 인식하지못해서 imread 불가능
        ## numpy로 이미지 불러오고 opencv로 변환
        img_array = np.fromfile(train_root_path + "[T원천]"+a[3]+"_"+a[4]+"_"+a[4]+"/"+a[5]+"/"+a[6][:-5]+".jpg", np.uint8)
        ...
        os.remove(train_root_path + "[T원천]"+a[3]+"_"+a[4]+"_"+a[4]+"/"+a[5]+"/"+a[6][:-5]+".jpg")
