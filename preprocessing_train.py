...
for idx, i in tqdm(enumerate(label_file_path[:600])):
    ...
    if file["Bounding"][0]["Drawing"] != "POLYGON":
        ...
        if (idx >= 301):
            cv2.imwrite("./300_image/"+image_file_path[idx].split("/")[2], img)
        else:
            cv2.imwrite("./0_image/"+image_file_path[idx].split("/")[2], img)
        w = int(img.shape[1])
        h = int(img.shape[0])
        ...
        if (idx >= 301):
            f = open('./300_label/'+name[:-4]+".txt", 'w')
        else :
            f = open('./0_label/'+name[:-4]+".txt", 'w')
